const About = () => {
  return (
    <section id="about">
      <div className="aboutBox">
        <div className="container">
          <div className="row cRows">
            <div className="col">
              <h1>We Are More</h1>
            </div>
            <div className="col">
              <p>
                Senja offers fast and affordable IT Services and bespoke Web-
                and App-Development. Our highly agile team will turn your vision
                into reality in budget and time. With over 7 years of experience
                and hundreds of successfully finished and running projects we
                are able to bring your product or service to Product-Market-Fit
                while providing you with in-depth advise on
                Go-To-Market-Strategies and effective, data-driven roadmapping.
              </p>
              <div className="row">
                <div className="col cCols">
                  <p>Strategy</p>
                  <ul class="list-group">
                    <li className="">Digital Strategy</li>
                    <li className="">Content Strategy</li>
                    <li className="">Responsive development</li>
                    <li className="">Web Design</li>
                    <li className="">Motion Design</li>
                    <li className="">Data Analyst</li>
                    <li className="">Brand Experience</li>
                  </ul>
                </div>
                <div className="col cCols">
                  <p>Development</p>
                  <ul class="list-group">
                    <li className="">Design Direction</li>
                    <li className="">Web & Mobile UI</li>
                    <li className="">Creative Concepts & Ideas</li>
                    <li className="">User Experience Design</li>
                    <li className="">Branding & Identity</li>
                    <li className="">Websites & Web Applications</li>
                    <li className="">E-Commerce</li>
                  </ul>
                </div>
              </div>
            </div>
          </div>
          <div className="betterDark">
            <div className="row cRows betterDark">
              <div className="col">
                <div className="headline-xxs">
                  <h1>We Do Better</h1>
                </div>
              </div>
              <div className="col">
                <b>Web Development</b>
                <div className="col">
                  <p>
                    From WordPress to Laravel, we code them all, front to back:
                    Corporate Homepages, Intranet Solutions, SaaS Websites and
                    Online Shops – for all browsers and devices.
                  </p>
                </div>
                <b>Game Development</b>
                <div className="col">
                  <p>
                    We are experts on Unity3D for games or just fun apps, be it
                    in 2D or 3D, Mulitplayer Online or Single Player based. We
                    work closely with top Artists and Game Designers to produce
                    high quality and entertaining experiences.
                  </p>
                </div>
                <b>Mobile Development</b>
                <div className="col">
                  <p>
                    We cover iOS and Android Apps, native, Web-based or the
                    likes of Xamarin. Along with all necessary Server work.
                  </p>
                </div>
                <b>Video Production</b>
                <div className="col">
                  <p>
                    Complex 2D and 3D-Animations made in and for AfterEffects,
                    we support SMEs in the Film-Business as well as YouTubers
                    who need help increasing the visual appeal of their
                    channels.
                  </p>
                </div>
                <b>Maintenance Services</b>
                <div className="col">
                  <p>
                    If you are running a successful Online Business or Website
                    and need support or leveled service, we are your best choice
                    to make sure it keeps running safe.
                  </p>
                </div>
              </div>
            </div>
          </div>

          {/* End */}
          <div className="row cRows">
            <div className="col-5">
              <h1>We Work</h1>
            </div>
            <div className="col">
              <div className="row ourSkill">
                <div className="plsTengah">
                  <div className="col aStroke">20+</div>
                  <div className="col">
                    <p>Skills</p>
                  </div>
                  <div className="col">
                    <ul class="list-group">
                      <li className="">ReactJS, NodeJS, AngularJS</li>
                      <li className="">HTML, Laravel, Wordpress</li>
                      <li className="">Obj-C, Swift, Java, ReactNative</li>
                      <li className="">Python, Unity3D</li>
                      <li className="">AfterEffects, Apple Motion</li>
                      <li className="">Customer Support</li>
                    </ul>
                  </div>
                </div>
              </div>
              {/* End */}
              <div className="row ourSkill2">
                <div className="plsTengah">
                  <div className="col aStroke">50+</div>
                  <div className="col">
                    <p>Team</p>
                  </div>
                  <div className="col">
                    <ul class="list-group">
                      <li className="">Web Developers</li>
                      <li className="">Mobile Developers</li>
                      <li className="">SAP Consultants</li>
                      <li className="">Motion Designers</li>
                      <li className="">Video Producers</li>
                    </ul>
                  </div>
                </div>
              </div>
              {/* End */}
              <div className="row ourSkill2">
                <div className="plsTengah">
                  <div className="col aStroke">300+</div>
                  <div className="col">
                    <p>Projects</p>
                  </div>
                  <div className="col">
                    <ul class="list-group">
                      <li className="">Offshore Development</li>
                      <li className="">Managed & Self Managed</li>
                      <li className="">Ad-hoc Prototyping & MVP'ing</li>
                      <li className="">Product Development</li>
                      <li className="">Planning & Consulting</li>
                    </ul>
                  </div>
                </div>
              </div>
              {/* End */}
              <div className="row ourSkill2">
                <div className="plsTengah">
                  <div className="col aStroke">10+</div>
                  <div className="col">
                    <p>Years</p>
                  </div>
                  <div className="col">
                    <ul class="list-group">
                      <li className="">IT Competency</li>
                      <li className="">Team Building & Up-Skilling</li>
                      <li className="">Handling wide range of clients</li>
                      <li className="">Challenges & Innovation</li>
                      <li className="">VC funded product spin-offs</li>
                    </ul>
                  </div>
                </div>
              </div>
              {/* End */}
            </div>
          </div>
          {/* End */}
        </div>
      </div>
    </section>
  );
};

export default About;
